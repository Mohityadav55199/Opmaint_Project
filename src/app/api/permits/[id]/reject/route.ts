...
import { RejectPermitSchema } from "@/lib/permit-schemas";
...
async function extractId(request: Request, context?: RouteContext): Promise<string> {
  ...
  const idx = segments.indexOf("reject");
...
export async function POST(
  ...
  try {
    ...
    const input = RejectPermitSchema.parse(payload);

    const updated = await rejectPermit(actor, id, input);
