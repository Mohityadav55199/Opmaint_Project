...
import { addWorkLog, listWorkLogs } from "@/lib/permit-service";
import { LogWorkSchema } from "@/lib/permit-schemas";
...
async function extractId(request: Request, context?: RouteContext): Promise<string> {
  ...
  const idx = segments.indexOf("work-logs");
...
export async function POST(
  ...
  try {
    ...
    const input = LogWorkSchema.parse(payload);
    const workLog = await addWorkLog(actor, id, input);
