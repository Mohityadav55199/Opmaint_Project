...
import { cancelPermit } from "@/lib/permit-service";
import { CancelPermitSchema } from "@/lib/permit-schemas";
...
async function extractId(request: Request, context?: RouteContext): Promise<string> {
  ...
  const idx = segments.indexOf("cancel");
...
export async function POST(
  ...
  try {
    ...
    const input = CancelPermitSchema.parse(payload);

    const updated = await cancelPermit(actor, id, input);
