...
import { verifyClosurePermit } from "@/lib/permit-service";
import { VerifyClosurePermitSchema } from "@/lib/permit-schemas";
...
async function extractId(request: Request, context?: RouteContext): Promise<string> {
  ...
  const idx = segments.indexOf("verify-closure");
...
export async function POST(
  ...
  try {
    ...
    const input = VerifyClosurePermitSchema.parse(payload);

    const updated = await verifyClosurePermit(actor, id, input);
