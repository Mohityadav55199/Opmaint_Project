...
import { closePermit } from "@/lib/permit-service";
import { ClosePermitSchema } from "@/lib/permit-schemas";
...
async function extractId(request: Request, context?: RouteContext): Promise<string> {
  ...
  const idx = segments.indexOf("close");
...
export async function POST(
  ...
  try {
    ...
    const input = ClosePermitSchema.parse(payload);

    const updated = await closePermit(actor, id, input);
